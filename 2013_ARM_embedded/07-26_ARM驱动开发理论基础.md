- 2013 - 07 - 26（ARM驱动开发理论基础）
***

# <span style="color:#ff0000;">1). 出错记录：
    ```python
    <1>在定义设备名称数组时不要使用dev_name，因其已经在include/linux/device.h:463中使用，会
            造成定义冲突
            eg: static char *dev_name[4] = {"led1", "led2", "led3", "led4"}        //错误方式
    <2>内核中的使用for循环的延时函数不起作用（？）
    ```
***

# <span style="color:#ff0000;">2). 嵌入式Linux用户空间与内核空间：
    ```python
    一个软件系统可以分为：应用程序、库、操作系统(内核)、驱动程序
    用户空间：应用程序(open/read/write)
            <---- 标准C库/系统调用函数(文件I/O，进程/线程，网络编程)(SWI指令)
    内核空间：系统调用 <---- VFS <---- 具体文件系统(磁盘文件系统、块设备文件系统) <---- 硬件驱动
            (字符设备、块设备(x盘/SD/nand)、网络设备)(设备文件，一切设备皆文件)
            VFS：给用户提供统一的接口，方便用户程序的开发
                    管理具体的文件系统，以及系统中的其他模块
            内核：协调管理各个功能模块(VFS，内存管理，进程管理/进程调度，网络管理，设备管理
                    (以设备驱动的方式管理各个设备，
                            字符设备驱动架构 ----> 具体的字符设备驱动
                            块设备驱动架构 ----> 具体的块设备驱动
                            socket层/网络设备层 ----> 具体的网络设备驱动))(截图)
                   提供各种服务
    ```
    <br />
  ...
  ![](images/052.png)
    ...
    linux内核七大子系统：
            ...
            内存管理：管理物理内存，虚拟内存的使用
            网络协议栈：
            VFS虚拟文件系统：屏蔽底层文件系统之间的差别，让用户在文  件的操作时候，无需关注具体
                    的文件系统类别和磁盘介质信息。
            平台相关（arch目录）
            设备驱动
 
    地址映射：
            <1> MMU将内存中物理地址映射到用户虚拟空间
 
            <2> ioremap将寄存器物理地址映射为虚拟地址(因为用户程序不能直接访问寄存器)
                    eg: gpg3con =ioremap(0xE03001C0, 8);
                        gpg3dat =gpg3con+1;
                    使用完后需要通过iounmap来释放
 
                使用 readl/writel readb/writeb来 读/写寄存器：
                    eg: static void __iomem *led_base = NULL;
            ...
            <3> 静态映射的虚拟地址：
                所有的管脚定义都定义在arch/arm/mach-s5pc100/include/mach/gpio.h中
                int gpio_request(unsigned fpio, const char *label);
    ...
    在Linux系统中，用户程序运行于用户空间，拥有MMU的系统能够限制应用程序的权限，这可以避免应用程序的
            错误使得整个系统的崩溃；而驱动程序运行于内核空间，其错误可能导致整个系统的崩溃。
 
    将数据拷贝到用户空间：copy_to_user / 获取用户空间的数据：copy_from_user
            //成功返回0，如果失败则返回大于0的值(剩下的没有拷贝成功的字节数)
    ```
    <br />
***

# <span style="color:#ff0000;">3). 嵌入式Linux环境搭建：
    ```python
    tftp协议用于下载内核映像，每次更新需要重新下载；相对于NFS协议，用于挂载文件系统，两边是同步的
    内核参数配置：console=ttySAC0,115200表示输出结果通过串口显示
            console=tty0,115200表示输出结果通过LCD屏显示
    dm9000中MAC地址更改：/arch/arm/plat-samsung下dev-dm9000.c文件中，更改
            struct dm9000_plat_data s5pc100_dm9000_platdata结构体 
    ```
    <br />
***

# <span style="color:#ff0000;">4). 设备驱动程序概述： &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
    ```python
    一切设备皆文件
    文件类型(7种)
    字符设备与块设备都是以文件的形式来操作
    字符设备与块设备：块设备的介质必须是存储介质，支持随机访问(对指定地址的访问)，系统调用
            操作lseek()是不可缺少的操作；而字符设备的介质一般都是传输介质，一般只支持顺序访问，
            所以，一般只是把成块输入/输出并且打算在上面建立普通文件系统的设备才成为块设备。
 
    所有的设备文件创建出来后都在/dev目录下面
            dev目录下设备文件属性：
                eg: crw-rw---- 1 root dialout 4, 64 7月 29 13:48 ttyS0
                    类型/权限          主/次设备号           设备名称
            设备号：        主设备号用来标识某一类设备，标识设备对应的驱动程序
...
# <span style="color:#ff0000;">5). 向系统(内核)中添加代码：
## <span style="color:#ff8000;">方法1：直接编译进内核镜像
  - 缺点：导致内核镜像很大，相对来说，系统启动会相对慢一些在开发过程中，需要每次都编译内核，浪费时间
### <span style="color:#00ff00;">&lt;1&gt; 将驱动添加到内核中，由系统启动的时候把驱动装好
  - I). 放入/driver中相应目录下(/driver/leds)
  - II). 修改该目录下的Makefile，在其中添加相应驱动的对应平台编译语句
    ```python
    eg: obj-$(CONFIG_LEDS_S5PC100)        += led-drv.o
    ```
    III). 修改该目录下的Kconfig文件，在图形化菜单上做一个菜单项
    ```python
    config LEDS_S5PC100        //表示一个菜单项，与前面的CONFIG_LEDS_S5PC100对应
            tristate "LED Support for S5pc100"        //在图形化菜单上显示的名称
    ...
    make menuconfig
    ```
### <span style="color:#00ff00;">&lt;2&gt; 将资源放到内核中，由系统统一管理
    ```python
    ...
    ```
### <span style="color:#00ff00;">&lt;3&gt; 编译内核：make zImage
