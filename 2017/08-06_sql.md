...
# 目录
  ...
  - [___2017 - 08 - 06 SQL___](#user-content-2017-08-06-sql)
  - [目录](#user-content-目录)
  - [SQL 简介](#user-content-sql-简介)
  - [mysql安装与导入 / 导出](#user-content-mysql安装与导入-导出)
  - [SQL 语法](#user-content-sql-语法)
  	- [SQL 语法基础](#user-content-sql-语法基础)
  	- [SQL SELECT 从数据库中选取数据](#user-content-sql-select-从数据库中选取数据)
  	- [SQL WHERE 子句过滤满足指定标准的记录](#user-content-sql-where-子句过滤满足指定标准的记录)
  	- [SQL ORDER BY 对结果集按照一个列或者多个列进行排序](#user-content-sql-order-by-对结果集按照一个列或者多个列进行排序)
  	- [SQL INSERT INTO 向表中插入新记录](#user-content-sql-insert-into-向表中插入新记录)
  	- [SQL UPDATE 更新表中已存在的记录](#user-content-sql-update-更新表中已存在的记录)
  	- [SQL DELETE 删除表中的记录](#user-content-sql-delete-删除表中的记录)
  	- [SQL JOIN 基于表的共同字段，结合两个或多个表的记录](#user-content-sql-join-基于表的共同字段结合两个或多个表的记录)
  	- [SQL ALTER TABLE 在已有的表中添加 / 删除 / 修改列](#user-content-sql-alter-table-在已有的表中添加-删除-修改列)
  	- [SQL UNION 合并两个或多个 SELECT 语句的结果](#user-content-sql-union-合并两个或多个-select-语句的结果)
  	- [SQL CREATE DATABASE / TABLE / INDEX 创建数据库 / 数据表 / 索引](#user-content-sql-create-database-table-index-创建数据库-数据表-索引)
  	- [SQL DROP DATABASE / TABLE / INDEX 撤销数据库 / 数据表 / 索引](#user-content-sql-drop-database-table-index-撤销数据库-数据表-索引)
  	- [SQL 约束 Constraints 规定表中的数据规则](#user-content-sql-约束-constraints-规定表中的数据规则)
  	- [SQL AUTO INCREMENT 新记录插入表中时生成一个唯一的数字，可用于主键](#user-content-sql-auto-increment-新记录插入表中时生成一个唯一的数字可用于主键)
  - [SQL 视图（Views）](#user-content-sql-视图views)
