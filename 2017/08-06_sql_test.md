...
# 目录
  ...
  - [目录](#目录)
  - [1. SQL 简介](#1-sql-简介)
  - [2. mysql安装与导入 / 导出](#2-mysql安装与导入-导出)
  - [3. SQL 语法](#3-sql-语法)
  	- [3.1 SQL 语法基础](#31-sql-语法基础)
  	- [3.2 SQL SELECT 从数据库中选取数据](#32-sql-select-从数据库中选取数据)
  	- [3.1 SQL WHERE 子句过滤满足指定标准的记录](#31-sql-where-子句过滤满足指定标准的记录)
  	- [3.2 SQL ORDER BY 对结果集按照一个列或者多个列进行排序](#32-sql-order-by-对结果集按照一个列或者多个列进行排序)
  	- [3.3 SQL INSERT INTO 向表中插入新记录](#33-sql-insert-into-向表中插入新记录)
  	- [3.4 SQL UPDATE 更新表中已存在的记录](#34-sql-update-更新表中已存在的记录)
  	- [3.5 SQL DELETE 删除表中的记录](#35-sql-delete-删除表中的记录)
  	- [3.6 SQL JOIN 基于表的共同字段，结合两个或多个表的记录](#36-sql-join-基于表的共同字段结合两个或多个表的记录)
  	- [3.7 SQL ALTER TABLE 在已有的表中添加 / 删除 / 修改列](#37-sql-alter-table-在已有的表中添加-删除-修改列)
  	- [3.8 SQL UNION 合并两个或多个 SELECT 语句的结果](#38-sql-union-合并两个或多个-select-语句的结果)
  	- [3.9 SQL CREATE DATABASE / TABLE / INDEX 创建数据库 / 数据表 / 索引](#39-sql-create-database-table-index-创建数据库-数据表-索引)
  	- [3.10 SQL DROP DATABASE / TABLE / INDEX 撤销数据库 / 数据表 / 索引](#310-sql-drop-database-table-index-撤销数据库-数据表-索引)
  	- [3.11 SQL 约束 Constraints 规定表中的数据规则](#311-sql-约束-constraints-规定表中的数据规则)
  	- [3.12 SQL AUTO INCREMENT 新记录插入表中时生成一个唯一的数字，可用于主键](#312-sql-auto-increment-新记录插入表中时生成一个唯一的数字可用于主键)
  - [4. SQL 视图（Views）](#4-sql-视图views)
...
## 3.2 SQL ORDER BY 对结果集按照一个列或者多个列进行排序
...
## 3.5 SQL DELETE 删除表中的记录
  ...
  - 删除所有数据，即不使用where子句删除表中所有的行，意味着表结构 / 属性 / 索引将保持不变
    ...
    1 row in set (0.00 sec)
    ```
## 3.6 SQL JOIN 基于表的共同字段，结合两个或多个表的记录
  - 不同的 SQL JOIN 方式
  ...
  - 示例 连接 websites 表与 access_log表，通过 websites 的 id 列与 access_log 的 site_id 列连接
    ...
    ```
## 3.7 SQL ALTER TABLE 在已有的表中添加 / 删除 / 修改列
  - SQL ALTER TABLE 语法
    - 在表中添加列
      ```sql
  ...
  - mysql中使用示例
    ...
    ```
## 3.8 SQL UNION 合并两个或多个 SELECT 语句的结果
  ...
  - SQL UNION 语法
    ...
    +---------+
    ```
## 3.9 SQL CREATE DATABASE / TABLE / INDEX 创建数据库 / 数据表 / 索引
  ...
  - **示例**
    ...
    ```
## 3.10 SQL DROP DATABASE / TABLE / INDEX 撤销数据库 / 数据表 / 索引
  - DROP DATABASE 删除数据库
    ```sql
  ...
  - DROP INDEX 删除表中的索引
    ...
    ```
## 3.11 SQL 约束 Constraints 规定表中的数据规则
  ...
  - 示例 DROP 撤销 PRIMARY KEY 约束
    ...
    ```
## 3.12 SQL AUTO INCREMENT 新记录插入表中时生成一个唯一的数字，可用于主键
