...
image_encoder.load_state_dict({kk[len('image_encoder.'):]: vv for kk, vv in ss.items() if kk.startswith('image_encoder.') and not kk.endswith('attention_bias_idxs')})
_ = image_encoder.eval()

import kecam
bb = 1024 / 32 / 7
mm = kecam.models.TinyViT_5M(input_shape=(1024, 1024, 3), strides=[1, 2, 2, 1], window_ratios=[bb * 8, bb * 4, bb, bb * 2], num_classes=0, pretrained=None)
...
torch_out = image_encoder.forward_features(torch.from_numpy(inputs).permute(0, 3, 1, 2)).detach().numpy().transpose([0, 2, 3, 1])
keras_out = mm(inputs)
print(f"{np.allclose(torch_out.reshape(keras_out.shape), keras_out, atol=9) = }")
print(f"{torch_out.min() = }, {torch_out.max() = }")
# torch_out.min() = -24.565926, torch_out.max() = 53.13518
```
...
## TinyViT
  ...
  print(np.allclose(torch_out.permute([0, 2, 3, 1]).detach().cpu(), keras_out, atol=3e-1))
  # True
  print(f"{torch_out.min() = }, {torch_out.max() = }")
...
## MaskDecoder
  ...
  # True
  print(np.allclose(low_res_masks.detach().permute([0, 2, 3, 1]), masks, atol=1e-4))
  # True
  ```
## PromptEncoder
  ```py
...
masks, _, _ = efficientvit_sam_predictor.predict(point_coords=point_coords, point_labels=point_labels, multimask_output=True)
# plt.imsave('aa.jpg', masks.transpose([1, 2, 0]).astype('uint8') * 255)
sam.SAM.show(image, masks.transpose([1, 2, 0]), points=point_coords, labels=point_labels, save_path='aa.jpg')
