...
class ModelEMA(kecam.backend.callbacks.Callback):
    ...
    def on_epoch_end(self, cur_epoch, logs=None):
        if hasattr(self.ema, "save_weights"):
            self.ema.save_weights(self.ema.name + "_ema.h5")
        elif hasattr(self.ema, "state_dict"):
            torch.save(self.ema.state_dict(), self.ema.__class__.__name__ + "_ema.pth")

class CloseMosaic(kecam.backend.callbacks.Callback):
...
class WarmupTrain(kecam.backend.callbacks.Callback):
    """
    import kecam
    ...
    fig.tight_layout()
    """
    def __init__(self, steps_per_epoch, warmup_epochs=3, warmup_momentum=0.8):
        self.steps_per_epoch, self.warmup_epochs, self.warmup_momentum = steps_per_epoch, warmup_epochs, warmup_momentum
    ...
    def on_train_batch_begin(self, batch, logs=None):
        ...
        for group_num, group in enumerate(self.model.optimizer.param_groups):
            warmup_lr = 0.1 if group_num == 0 else 0
...
class LossMeanMetricWrapper(kecam.backend.metrics.Metric):
...
box_loss_metric = LossMeanMetricWrapper(loss, loss_attr_name="box_loss")
cls_loss_metric = LossMeanMetricWrapper(loss, loss_attr_name="cls_loss")
dfl_loss_metric = LossMeanMetricWrapper(loss, loss_attr_name="dfl_loss")
...
warmup_train = WarmupTrain(steps_per_epoch=len(train_dataset), warmup_epochs=warmup_epochs)
close_mosaic = CloseMosaic(train_dataset, close_mosaic_epoch=epochs - 10)
