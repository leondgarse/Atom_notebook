## YOLOV8
  ...
  seg_headers = [
      'seg_head_1_reg_1_conv', 'seg_head_1_reg_1_bn', 'seg_head_1_reg_2_conv', 'seg_head_1_reg_2_bn', 'seg_head_1_reg_3_conv',
      'seg_head_2_reg_1_conv', 'seg_head_2_reg_1_bn', 'seg_head_2_reg_2_conv', 'seg_head_2_reg_2_bn', 'seg_head_2_reg_3_conv',
      'seg_head_3_reg_1_conv', 'seg_head_3_reg_1_bn', 'seg_head_3_reg_2_conv', 'seg_head_3_reg_2_bn', 'seg_head_3_reg_3_conv',
      'seg_head_1_cls_1_conv', 'seg_head_1_cls_1_bn', 'seg_head_1_cls_2_conv', 'seg_head_1_cls_2_bn', 'seg_head_1_cls_3_conv',
      'seg_head_2_cls_1_conv', 'seg_head_2_cls_1_bn', 'seg_head_2_cls_2_conv', 'seg_head_2_cls_2_bn', 'seg_head_2_cls_3_conv',
      'seg_head_3_cls_1_conv', 'seg_head_3_cls_1_bn', 'seg_head_3_cls_2_conv', 'seg_head_3_cls_2_bn', 'seg_head_3_cls_3_conv',
      'seg_head_mask_protos_1_conv', 'seg_head_mask_protos_1_bn', 'seg_head_mask_protos_up', 'seg_head_mask_protos_2_conv',
      'seg_head_mask_protos_2_bn', 'seg_head_mask_protos_3_conv', 'seg_head_mask_protos_3_bn',
      'seg_head_1_mask_coefficients_1_conv', 'seg_head_1_mask_coefficients_1_bn', 'seg_head_1_mask_coefficients_2_conv',
      'seg_head_1_mask_coefficients_2_bn', 'seg_head_1_mask_coefficients_3_conv',
      'seg_head_2_mask_coefficients_1_conv', 'seg_head_2_mask_coefficients_1_bn', 'seg_head_2_mask_coefficients_2_conv',
      'seg_head_2_mask_coefficients_2_bn', 'seg_head_2_mask_coefficients_3_conv',
      'seg_head_3_mask_coefficients_1_conv', 'seg_head_3_mask_coefficients_1_bn', 'seg_head_3_mask_coefficients_2_conv',
      'seg_head_3_mask_coefficients_2_bn', 'seg_head_3_mask_coefficients_3_conv',
  ]
  headers = seg_headers if mm.name.endswith("_seg") else det_headers
  specific_match_func = lambda tt: tt[:- len(headers)] + headers
  if mm.name.endswith("_seg"):
      _ = ss.pop([ii for ii in ss if ii.endswith('.dfl.conv.weight')][0])
  ...
  **Convert bboxes output `[left, top, right, bottom]` -> `top, left, bottom, right`**
  ...
  head_prefix = "seg_head" if mm.name.endswith("_seg") else "head"
  heads = [ii.name for ii in mm.layers if ii.name.startswith(head_prefix)]
  for ii in range(1, 100):
      layer_name = '{}_{}_reg_3_conv'.format(head_prefix, ii)
      print(f">>>> {layer_name = }")
...
## YOLOV8 Segmentation
  ...
  sys.path.append('../ultralytics')
  from ultralytics import YOLO
  from keras_cv_attention_models import yolov8, coco, test_images

  """ Torch output """
  model = YOLO('yolov8n-seg.pt')  # load an official model
  rr = model(test_images.dog_cat()[:, :, ::-1])
  rr[0].save('aa.jpg')
  print(f"{model.predictor.args.conf = }, {model.predictor.args.iou = }, {model.predictor.args.max_det = }")
  # model.predictor.args.conf = 0.25, model.predictor.args.iou = 0.7, model.predictor.args.max_det = 300

  """ Keras output """
  from keras_cv_attention_models import yolov8, test_images, coco, plot_func
  mm = yolov8.YOLOV8_N_SEG(pretrained='yolov8_n_seg_coco.h5')
  image = test_images.dog_cat()
  preds, mask_protos = mm.predict(mm.preprocess_input(image))
  bboxes, labels, confidences, masks = mm.decode_predictions(preds, mask_protos=mask_protos, score_threshold=0.25, iou_or_sigma=0.7)[0]
  ax = plot_func.show_image_with_bboxes(image, bboxes, labels, confidences, masks=masks)
  ax.get_figure().savefig('bb.jpg')
  ```
  ```py
  ...
  print(f'{torch_out[0].shape = }\n{[ii.shape for ii in torch_out[1][0]] = }\n{torch_out[1][1].shape = }\n{torch_out[1][2].shape = }')
  # torch_out[0].shape = torch.Size([1, 116, 8400])
  # [ii.shape for ii in torch_out[1][0]] = [torch.Size([1, 144, 80, 80]), torch.Size([1, 144, 40, 40]), torch.Size([1, 144, 20, 20])]
  # torch_out[1][1].shape = torch.Size([1, 32, 8400])
  # torch_out[1][2].shape = torch.Size([1, 32, 160, 160])
  ...
  print(f"{keras_out.shape = }, {keras_masks.shape = }")
  # keras_out.shape = TensorShape([1, 8400, 176]), keras_masks.shape = TensorShape([1, 160, 160, 32])
...
# Data
  ...
  dataset_path = "datasets/coco_dog_cat/ultralytics.yaml"
  ...
  plt.imsave('aa.jpg', aa['img'][0].permute([1, 2, 0]).numpy())

  for aa in tqdm(train_loader):
      pass
  # 519/519 [00:29<00:00, 17.59it/s]

  """ close_mosaic """
  cfg = ultralytics.cfg.get_cfg(ultralytics.utils.DEFAULT_CFG)
  train_loader.dataset.close_mosaic(cfg)
  aa = next(iter(train_loader))
  plt.imsave('aa.jpg', aa['img'][0].permute([1, 2, 0]).numpy())
  for aa in tqdm(train_loader):
    pass
  # 519/519 [00:27<00:00, 18.98it/s]
  ```
  Time kecam
  ```py
  os.environ["KECAM_BACKEND"] = "torch"
  ...
  train, test = torch_data.init_dataset('datasets/coco_dog_cat/detections.json', batch_size=16)
  train.dataset.mosaic = 1.0
  aa = next(iter(train))

  for ii in tqdm(train):
      pass
  # 519/519 [00:34<00:00, 15.00it/s]

  train.dataset.mosaic = 0
  for ii in tqdm(train):
      pass
  # 519/519 [00:41<00:00, 12.39it/s]
  ```
***
...
## Convert
  ...
  if use_reparam_conv:
      ss = {kk: vv for kk, vv in tt.state_dict().items() if not ('rbr_reparam' in kk)}
  else:
  ...
  mm = yolo_nas.YOLO_NAS_L(pretrained=None, use_reparam_conv=use_reparam_conv)

  from keras_cv_attention_models import download_and_load
  from keras_cv_attention_models import attention_layers

  tail_align_dict = {"middle_bn": -1} # , "short_0_alpha": "deep_bn"}
  tail_align_dict.update({"short_{}_alpha".format(ii): "pre_{}_1_conv".format(ii) for ii in range(0, 10)})

  full_name_align_dict = {"stack4_spp_fast_output_bn": -1, "pafpn_p3p4p5_output_bn": -1}
  if not use_reparam_conv:
      parallel_stacks = {"s": [], "m": [1, 2, 3], "l": [1, 2, 3, 4]}[mm.name[-1]]
      num_blocks = [2, 3, 5, 2]
  ...
  **Convert bboxes output `[left, top, right, bottom]` -> `top, left, bottom, right`**
  ...
  from keras_cv_attention_models.yolov8 import yolo_nas
  use_reparam_conv = True
  mm = yolo_nas.YOLO_NAS_L(pretrained="yolo_nas_l_imagenet.h5", use_reparam_conv=use_reparam_conv)
  ...
  bb = keras.models.load_model(mm.name + '_imagenet.h5')
  for ii in mm.layers:
...
loss = kecam.coco.losses.Loss(device=global_device, nc=num_classes)
...
""" Comile """
grad_accumulate = max(round(64 / batch_size), 1)
...
""" Callback """
warmup_train = kecam.imagenet.callbacks.WarmupTrain(steps_per_epoch=len(train_dataset), warmup_epochs=warmup_epochs)
close_mosaic = kecam.imagenet.callbacks.CloseMosaic(train_dataset, close_mosaic_epoch=epochs - 10)
coco_ap_eval = kecam.coco.eval_func.COCOEvalCallback(
...
learning_rate_scheduler = lambda epoch: lr * ((1 - epoch / epochs) * (1.0 - lr) + lr)  # linear
