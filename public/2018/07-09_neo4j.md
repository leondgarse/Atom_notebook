...
## Graph
  ...
  - **create** 方法传入 Subgraph 对象来将关系图添加到数据库中
    ...
    graph.create(s)
    ```
    **Graph.create 实际执行的是 Transaction.create**
    ```python
    ...
    **清除** 清除后如果要重新创建，需要重新初始化 Node / Relationship
    ...
    ```
  - **match / match_one** 查找 Relationship
    ```python
    graph.match_one(r_type='KNOWS')
  ...
  - **nodes.match** 查找 Node
    ...
    ```
    ```python
    list(graph.nodes.match("Person"))
    # Out[94]: [(_105:Person {name: 'Alice'}), (_106:Person {name: 'Bob'})]
    ```
  - **relationships.match** 查找 Relationship
    ...
    # nodes is a list
    graph.relationships.match(nodes=[aa], r_type="KNOWS").first()
    # Out[108]: (Alice)-[:KNOWS {}]->(Bob)
    ```
  - **delete** 删除子图，实际执行 `Transaction.delete`
    - 删除 **节点** 同时会删除所有对应的 **关系**
    - 删除 **关系** 同时会删除所有对应的 **节点**
    ```python
  ...
  - **push** 更新 Node 属性
    ...
    # Out[123]: (_107:Person {age: 21, name: 'Alice'})
    ```
  - **run** 直接执行 CQL 命令
    ```python
    [tt.data() for tt in graph.run('MATCH (p:Person) return p')]
    ...
    ```
## OGM
  - **OGM** Object Graph Mapping，类似于 ORM，可以实现一个对象和 Node 的关联
    ```python
  ...
  - **match / where** 结合 Graph 查询
    ...
    # pp = <Person name='Alice'>, pp.name = Alice, pp.age = 21

    ''' 使用 Object 更新数据 '''
    pp.__ogm__.node
    # Out[5]: (_114:Person {age: 21, name: 'Alice'})

    pp.age = 22
    pp.location="Qing Dao"
    pp.__ogm__.node
    # Out[7]: (_114:Person {age: 22, location: 'Qing Dao', name: 'Alice'})

    graph.push(pp)
    graph.nodes.match("Person").first()
    # Out[9]: (_114:Person {age: 22, location: 'Qing Dao', name: 'Alice'})
    ```
  - **RelatedTo** 查看 / 设置关系
    ```python
