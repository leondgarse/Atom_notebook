...
## Layui issues
  - **application.py**
    ...
    #!/usr/bin/env python
    from flask import Flask, render_template
    app = Flask(__name__, template_folder='views', static_folder='layuiadmin')
    ...
    def _favicon_():
        with open('favicon.ico', 'rb') as ff:
            return ff.read()
  ...
  - **Q / A**
    ```py
    ''' Q: jinja2.exceptions.TemplateNotFound: favicon.ico
    '''
    ''' A: Add `favicon.ico` in the same path with `application.py`.
           Then add in `views/index.html` `<head>` section.
           $ vi views/index.html
           +13   <link rel="shortcut icon" href="#" />

           Or add '/favicon.ico' route:
           @app.route('/favicon.ico', methods=["GET", "POST"])
           def _favicon_():
               with open('favicon.ico', 'rb') as ff:
                   return ff.read()
    ...
    ```
    ```py
    ''' Q: jinja2.exceptions.UndefinedError: 'layui' is undefined
    '''
    ''' A: `{{ }}` grammar conflicts in `jinjia2` and `layui`, change `layui` grammar to a string.
           $ vi views/home/console.html
           -229                       v{{ layui.admin.v }}
           +229                       {{ "v{{ layui.admin.v }}" }}
           -238                       layui-v{{ layui.v }}
           +238                       {{ "layui-v{{ layui.v }}" }}
